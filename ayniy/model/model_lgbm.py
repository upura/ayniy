...
class ModelLGBM(Model):
    def train(self, tr_x, tr_y, va_x=None, va_y=None, te_x=None):
        ...
        if validation:
            ...
            self.model = lgb.train(params, lgb_train, num_round, valid_sets=[lgb_train], verbose_eval=500)
    ...
    def train(self, tr_x, tr_y, va_x=None, va_y=None, te_x=None):
        ...
        if validation:
            lgb_eval = optuna_lgb.Dataset(
                va_x, va_y, reference=lgb_train, categorical_feature=self.categorical_features, free_raw_data=False
            )
